...
async def callback_service(call: CallbackQuery, state: FSMContext):
    try:
        ...
        try:
            allowed_categories = {"Техническая помощь / Technical Support", "NFA / HWID RESET"}
            service_name = add_order.get('service_name')

            if service_name in allowed_categories:
                enabled = await redis_client.get("autoaccept:enabled")
                owner = await redis_client.get("autoaccept:owner")
                enabled = str(enabled) if enabled is not None else None
                owner = str(owner) if owner is not None else None

                if enabled in {"1", "true", "True"} and owner == "434791099":
                    active_cnt = await db.count_active_for(434791099)
                    if int(active_cnt or 0) == 0:
                        reserve_key = f"autoaccept:reserve:{add_order['id']}"
                        reserved = await redis_client.set(reserve_key, owner, ex=3, nx=True)
                        if reserved:
                            accept = await db.accept_orders(add_order['id'], 434791099)
                            if accept and accept not in ('Пользователь не имеет роли!', 'Active-Ticket', 'Not-New', False):
                                try:
                                    if accept.service_name == "Техническая помощь / Technical Support":
                                        ...
                                        await call.bot.send_message(
                                            ...
                                            text=(
                                                "Приветствую!\n"
                                                "Для сброса HWID предоставь информацию по форме:\n\n"
                                                "1. <u><b>Скриншот</b></u>, подтверждающий покупку в личном кабинете "
                                                "<u>(ключ должно быть видно на скриншоте)</u>\n\n"
                                                "2. Ключ продукта <u>в текстовом формате</u>\n\n"
                                                "3. Используешь сторонний спуфер (не встроенный в чит)?"
                                            ),
                                            parse_mode="HTML"
                                        )
                                except Exception as e:
                                    logger.warning(f"Autoaccept: failed to greet client: {e}")

                                try:
                                    from aiogram.types import ReplyKeyboardMarkup, KeyboardButton
                                    await call.bot.send_message(
                                        chat_id=434791099,
                                        text=f"Тикет №{add_order['id']} принят!\nЧат с пользователем открыт!",
                                    ...
                                    logger.warning(f"Autoaccept: failed to notify developer: {e}")
                                ...
                                try:
                                    ...
                                    message_accept = (
                                        "✅ Тикет принят!\n\n"
                                        f"📩 <b>Тикет</b> №{add_order['id']}\n"
                                        f"👤 <b>Пользователь:</b> @{_html.escape(accept.client_name)}\n"
                                        f"🆔 <b>ID:</b> {accept.client_id}\n"
                                        f"<a href=\"https://t.me/{_html.escape(accept.client_name)}\">🔗 1.Телеграм</a>\n"
                                        f"<a href=\"tg://user?id={accept.client_id}\">🔗 2.Телеграм</a>\n"
                                        f"🛠 <b>Услуга:</b> {_html.escape(accept.service_name)}\n"
                                        f"🆔 <b>Support_id:</b> {accept.support_id}\n"
                                        f"👨‍💻 <b>Support_name:</b> @{_html.escape(accept.support_name)}\n"
                                        f"ℹ️ <b>Статус:</b> {_html.escape(accept.status)}\n"
                                    ...
                                    logger.warning(f"Autoaccept: cannot edit support post: {e}")

                                try:
                                    await redis_client.set(f"ticket:{accept.support_id}", add_order['id'])
                                except Exception:
                                    pass

                                try:
                                    from handlers.Worker.Start import auto_close_ticket_if_silent, active_timers
                                    task = asyncio.create_task(
                                        auto_close_ticket_if_silent(add_order['id'], accept.client_id, call.bot)
                                    )
                                    active_timers[add_order['id']] = task
                                except Exception as e:
                                    logger.warning(f"Autoaccept: failed to start auto-close timer: {e}")
            ...
            logger.warning(f"Autoaccept: unexpected error for order {add_order.get('id')}: {e}")
...
async def start_accounts(message: Message):
    ...
    await message.answer("Choose account type" if lang == "en" else "Выберите тип аккаунта", reply_markup=accounts_kb(lang))
...
async def zero_accounts(call: CallbackQuery):
    ...
    if lang == "en":
        message_text = (
            "<b>🎮 Rust account</b> with random hours <b>🕒 0–100</b>\n\n"
            "<b>❗️ Important:</b>\n"
            "<u>Login:password is provided</u>\n"
            "⛔️ It is <u>forbidden to change registration data</u> "
            "(<b>password/email/phone</b>) — this will lead to <b>permanent ban</b> "
            "without exchange or refund 🔐\n\n"
            "✅ <b>Allowed to change:</b>\n"
            "• 📝 Profile\n"
            "• 📸 Photo\n"
            "• 🔤 Name\n\n"
            "<b>⚙️ Required:</b>\n"
            "🚫 In settings <b>DISABLE</b> Remote play\n"
            "💵 Price: 800₽"
        )
        buy_now = "🛒 Buy now"
        back = "⬅️ Back"
        ...
        message_text = (
            "<b>🎮 Раст аккаунт</b> с рандомными часами <b>🕒 от 0 до 100</b>\n\n"
            "<b>❗️ ВАЖНОЕ ОПИСАНИЕ:</b>\n"
            "<u>Выдается логин:пароль</u>\n"
            "⛔️ На аккаунте <u>запрещено менять регистрационные данные</u> "
            "(<b>пароль/почту/телефон</b>) — это приведёт к <b>полной блокировке</b> "
            "аккаунта <u>без обмена и возврата</u> 🔐\n\n"
            "✅ <b>Разрешено изменять:</b>\n"
            "• 📝 Профиль\n"
            "• 📸 Фотографию\n"
            "• 🔤 Имя\n\n"
            "<b>⚙️ Обязательное условие:</b>\n"
            "🚫 В настройках <b>ОТКЛЮЧИТЬ</b> Remote play\n"
            "💵 Цена: 800₽"
        )
...
async def active_accounts(call: CallbackQuery, bot: Bot):
    ...
    message_text_en = (
        "<b>🎮 Rust account</b> with <b>🕒 1500+ hours + Twitch items</b>\n\n"
        "<b>❗️ Important:</b>\n"
        "<u>Login:password is provided, 3 days warranty, email is given after warranty period of 72 hours</u>\n"
        "⛔️ Before the warranty ends it is <u>forbidden to change registration data</u>"
        "(<b>password/email/phone</b>) — this will lead to <b>permanent ban</b> "
        "without exchange or refund 🔐\n\n"
        "✅ <b>Allowed to change:</b>\n"
        "• 📝 Profile\n"
    ...
    message_text_ru = (
        "<b>🎮 Раст аккаунт</b> с часами <b>🕒 от 1500 + твич предметы</b>\n\n"
        "<b>❗️ ВАЖНОЕ ОПИСАНИЕ:</b>\n"
        "<u>Выдается логин:пароль, 3 дня гарантии, почта отдается - после окончания гарантии сроком в 72 часа</u>\n"
        "⛔️ До истечения срока гарантии на аккаунте <u>запрещено менять регистрационные данные</u>"
        "(<b>пароль/почту/телефон</b>) — это приведёт к <b>полной блокировке</b> "
        "аккаунта <u>без обмена и возврата</u> 🔐\n\n"
        "✅ <b>Разрешено изменять:</b>\n"
        "• 📝 Профиль\n"
...
async def accounts_back(call: CallbackQuery):
    ...
    await call.message.delete()
    await call.message.answer("Choose account type" if lang == "en" else "Выберите тип аккаунта", reply_markup=accounts_kb(lang))
